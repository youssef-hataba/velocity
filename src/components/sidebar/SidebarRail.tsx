...
export const SidebarRail = ({ projects, activeProjectId, onSelectProject }: SidebarRailProps) => {
  ...
  return (
    <div className="w-18 flex flex-col items-center py-8 border-r border-steel/10 bg-secondary/5 backdrop-blur-3xl z-30 gap-8 shrink-0">
      <div className="w-12 h-12 rounded-2xl overflow-hidden shadow-2xl shadow-primary/40 group cursor-pointer 
      transition-all hover:scale-105 active:scale-95">
        <img src="/favicon.png" alt="Logo" className="object-contain w-full h-full" />
      </div>

      <div className="flex flex-col gap-5 flex-1 no-scrollbar py-2 px-3">
        {projects.map((p) => (
          <WorkspaceIcon
            key={p.id}
            project={p}
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-100 rounded-[2.5rem] bg-card/90 backdrop-blur-2xl border-white/10 shadow-2xl focus:outline-none border-none">
            <DialogHeader>
              <DialogTitle className="text-2xl font-black tracking-tighter text-center uppercase italic">
                New Project
              </DialogTitle>
            </DialogHeader>
      ...
      <button className="text-muted/40 hover:text-foreground transition-all p-3 hover:bg-secondary/50 rounded-xl group">
