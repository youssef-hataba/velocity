    ...
    <div className="lg:col-span-8 space-y-12">
      ...
      <div className="pt-10 flex items-center gap-4 border-t border-white/5">
        <button 
          ...
          className="flex-1 bg-primary hover:bg-primary/90 text-black h-16 rounded-4xl font-black uppercase tracking-widest text-[12px] flex items-center justify-center gap-3 transition-all active:scale-95"
        >
          {props.isNew ? (
            <><Plus size={20} strokeWidth={3} /> Launch Mission</>
          ) : (
            <><CheckCircle2 size={20} strokeWidth={2} /> Save & Exit</>
          )}
        </button>
        ...
        {!props.isNew && (
          <button 
            ...
            className="p-5 rounded-[1.8rem] bg-red-500/5 hover:bg-red-500/10 border border-red-500/20 text-red-500/60 hover:text-red-500 transition-all active:scale-95"
          >
            <Trash2 size={22} />
          </button>
        )}
